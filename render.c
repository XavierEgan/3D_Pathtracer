...
void render(Camera cam, Mesh* meshs, unsigned int num_meshs, char* filename) {
    ...
    float vertical_half_scale = tan(cam.height_fov * .5f) * cam.focal_length;
    float horizontal_half_scale = tan(cam.width_fov * .5f) * cam.focal_length;
                ...
                for (unsigned int rt=0; rt<cam.num_bounces; rt++) {
                    ...
                    if (closest_intercept.tri.emmisive) {
                        ...
                        if (closest_intercept.tri.reflective) {
                            ...
                            ray = vec_from_spherical(rand()*inv_max_rand*2*3.1415926535f, rand()*inv_max_rand*2*3.1415926535f);
